...
export const voteForCandidate = function({candidate,email,Registrar,Voting}) {
  ...
  axios.get(url).then(function(response) {
    Voting.at(votingAddress).then(function(contract) {
      contract.votesFor.call(hcHash).then(function(v) {
          ...
          if (einput1 != 0) {
              add(response, einput1, i, candidateArray, email, votingAddress, votesArray)
          }
      })
    })   
  })
...
function add(eoutput1, einput1, i, candidateArray, email, votingAddress, votesArray) {
  ...
  axios.get(url).then(function(response){
    verifyTimestamp(response, i, candidateArray, email, votingAddress, votesArray)
  })
  .catch(function(error){
    console.log(error);
  });
...
export const ballotSetup = function({cemail, date, time, ballottype, title, choices, votelimit, whitelist, whitelisted, Registrar, Creator}) {
